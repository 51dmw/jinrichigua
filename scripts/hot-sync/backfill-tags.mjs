...
const DEEP_REJECT = [
  ...
  { t: '王菊', tag: '追星' },
  { t: '阿梓不可爱', tag: '追星' },
  // ↓ 恢复词表后重跑 deep 的第二批否决（词库变大后精确率从 55% 升到 92%，只剩这 3 处）
  { t: '汪苏泷', tag: '盘点' },           // 单一事件，不是盘点文
  { t: '刘宇宁', tag: '聊天' },           // 文中没有聊天记录
  { t: 'Jennie', tag: '控评' },           // 文中没有控评情节
];

// 泛词：这些已经挂得到处都是，不需要模型再产出
const GENERIC = new Set([
  '热搜', '网络热议', '吃瓜', '围观', '出圈', '反转', '大瓜', '黑料', '实锤',
  '爆料', '视频', '微博热搜', '热榜', '娱乐八卦', '明星', '冲突', '塌房', '顶流', '赛后', '网红',
  // ↓ 恢复 tag-vocabulary.json 后新增的一批。它们跟上面那些是同一类造势词，
  //   不挡住的话模型会拿它们去换掉现有泛词——实测产出过
  //   「老外体验中国式避暑游 → 疯传,全网」「超五年不联系就删 → 热议,话题榜」，
  //   换完还是没有具体信息，等于白换。
  '头条', '热点', '热门', '热议', '刷屏', '爆火', '关注', '疯传', '霸榜', '冲榜', '上榜',
  '爆款', '趋势', '焦点', '全网', '热传', '新瓜', '旧瓜', '爆瓜', '猛料', '爆文', '流量王',
  '话题榜', '热搜榜', '排行榜', '热议榜', '事件榜', '热门榜', '热点榜', '推荐榜',
  '热搜词', '买热搜', '降热搜', '热搜瓜', '舆论场', '关键词', '刷量', '营销号',
  // ↓ 词表里一批生造的「X曝」，不是正常会用的标签词
  '视频曝', '截图曝', '猛料曝', '后续曝', '录音曝', '现场曝', '网络曝', '合同曝', '邮件曝',
  '匿名曝', '高管曝', '企业曝', '粉丝曝', '经典曝', '历史曝', '站姐曝', '老板曝', '同事曝',
  '公司曝', '行业曝', '同行曝', '实名曝', '聊天曝', '员工曝', '海外曝', '国外曝', '国外黑',
]);
